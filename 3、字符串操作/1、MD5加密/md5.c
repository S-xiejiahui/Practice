 ...
 *
 * @param input_string：   待加密的数据(数据长度应小于64)
...
{
    if (NULL == input_string || NULL == output_md5_string)
    {
        return;
    }
    ...
    MD5_CTX md5;
    MD5Init(&md5);
    MD5Update(&md5, input_string, strlen((char *)input_string));
    MD5Final(&md5, encrypt_data);

    for (i = 0; i < 16; i++)
    {
        s_offset += snprintf(output_md5_string + s_offset, md5_string_len - s_offset, "%02x", encrypt_data[i]);
    }
    return;
}
...
{
    if (NULL == normal_string || NULL == md5_string)
    {
        return -1;
    }
    char encrypt_data[128] = {0};
    MD5_Encrypt_String(normal_string, encrypt_data, sizeof(encrypt_data));
    return strncmp(encrypt_data, md5_string, 32);
}
...
int main(int argc, char *argv[])
{
    ...
    int ret = string_compared_with_md5_data("tenda_mfg", "f9997889d89aae8092b4e4d9dd237274");
    printf("ret = %d\n", ret);
    return 0;
}