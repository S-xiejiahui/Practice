...
:: ------------------------------------- 准备工作 -------------------------------------------
:: 搭配最下方脚本，以图像界面的方式获取选择的文件夹的绝对路径
set target_file=
for /f "delims=" %%a in ('mshta "%~f0"') do set target_file=%%a

:: 如果上面没有选择文件，则直接退出脚本
if not exist "%target_file%" (
	echo.
    echo  -----[ File selection error, please select the correct path ]-----
	echo.
	pause
)
echo.
echo  -----[ choose %target_file% ]-----
...
for %%p in (%product_name%) do (
	...
	if not exist %pclint_report_dir%\%%p (
		...
		del %pclint_report_dir%\%%p\*.xml /q >nul 2>nul
	)

	:: 将所有 error 和 warning 汇总到 all.xml 中
	echo ^<^?xml version="1.0" encoding="UTF-8"^?^> >> %pclint_report_dir%\%%p\all.%file_type%
	echo ^<doc^> >> %pclint_report_dir%\%%p\all.%file_type%
	...
	:: 所有 option
		set all_option=%system_option% -u %public_option% !file_self_option_rule! !check_head! %prod_platform_inc% !check_c_file!

	:: 检测
		%pclint_install_dir%\lint-nt.exe !all_option! >> %pclint_report_dir%\%%p\%%j.%file_type%
		echo.
	:: 每检测一个文件，就打印出 error warning 个数
		call :get_error_warning %pclint_report_dir%\%%p\%%j.%file_type% ErrorNumer WarningNumer
		echo ----------------------------------------------------------------------
		echo %tab%%%j   %tab%{  Error: !ErrorNumer!, %tab%Warning: !WarningNumer!  }
		echo ----------------------------------------------------------------------
	:: 将单个 xml 中的内容汇总到 all.xml 中
		findstr .* %pclint_report_dir%\%%p\%%j.%file_type%|findstr /vb "<doc>"|findstr /v "</doc>"|findstr /v "<?xml version="1.0" encoding="UTF-8"?>" >> %pclint_report_dir%\%%p\all.%file_type%
		echo.
	)
...
:: 检测 error 和 warning 的个数
	call :get_error_warning %pclint_report_dir%\%%p\all.%file_type% ErrorNumer WarningNumer
	echo.
	echo ===========================    %%p    ===================================
	echo.
...
echo. > %pclint_filelist_dir%\%filename%_inc.lnt
cd !absolute_path!
...
<script>
var Message = "\u8bf7\u9009\u62e9\u6587\u4ef6\u5939";	// '请选择文件夹' 的编码
var Shell = new ActiveXObject("Shell.Application");
var Folder = Shell.BrowseForFolder(0x00, Message, 0x40, 0x11); //起始目录为：计算机(0x11),只显示文件夹(0x40)
if (Folder != null) {
    Folder = Folder.items();
    Folder = Folder.item();
    Folder = Folder.Path;
